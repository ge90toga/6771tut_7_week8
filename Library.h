...
void Library<T,U>::addRelated(const T& from, const T& to, const U& desc) {
    ...
    // add desc
    //what if (*fromIC).addRelated(*toIC,desc)?
...
unsigned int Library<T,U>::remove(const T& item) {
    auto to_be_removed = std::find_if(items.begin(),items.end(),[&item] (const ItemContainer &ic){
        return item == ic.getItem();
    });
    if(to_be_removed!=items.end()) items.erase(to_be_removed);
    return items.size();
}
